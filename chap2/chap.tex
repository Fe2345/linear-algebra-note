			...
			\begin{para}{0}
				\point{}
					...
					\begin{proof}
						...
						我们先定义：$E_{ij}:V \ni \sum\limits_{l=1}^{n} k_l \alpha_l \mapsto k_i \alpha_j \in V$。容易注意到，$\{E_{ij}\}$是$\hom(V,V)$的一个基

						考虑映射：$f: \hom(V,V) \ni T = \sum\limits_{i,j}^{n,n} a_{ij} E_{ij} \mapsto \sum\limits_{k=1}^{n} a_{kk} \in F$

						注意到：$\forall T=\sum\limits_{i,j}^{n,n}a_{ij}E_{ij},S=\sum\limits_{i,j}^{n,n}b_{ij}E_{ij} \in \hom(V,V)$

						$f(TS)=f\left(\sum\limits_{i,j}^{n,n}\left(\sum\limits_{k=1}^{n} a_{ik} b_{kj}\right)E_{ij}\right)=\sum\limits_{i=1}^{n}\sum\limits_{k=1}^{n}a_{ik}b_{ki}$
